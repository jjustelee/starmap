    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-2 text-center pt-4">
                <h2 className="text-4xl font-black text-white tracking-tight">성지글 갤러리</h2>
            ...
            <div className="flex gap-2">
                <div className="relative flex-1 group">
                    ...
                    <input
                        type="text"
                        placeholder="성지글 검색..."
                        className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-10 pr-4 text-sm font-bold text-white focus:outline-none focus:border-neon-teal/50 transition"
                    />
                </div>
                <button className="rounded-2xl bg-white/5 border border-white/10 p-3 text-white/50 hover:text-white transition">
            ...
            <div className="flex gap-2 p-1 bg-white/5 rounded-2xl border border-white/5">
                <button
                    ...
                    className={`flex-1 py-3 text-sm font-black rounded-xl transition ${filter === 'recent' ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`}
                >
                    최근 적중
                </button>
                <button
                    onClick={() => setFilter('popular')}
                    className={`flex-1 py-3 text-sm font-black rounded-xl transition ${filter === 'popular' ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`}
                >
                    많이 순례한 글
                </button>
            </div>
                ...
                {posts.map((post) => (
                    ...
                    >
                        <div className="relative">
                            ...
                            <h4 className="text-2xl font-black tracking-tight text-white group-hover:text-neon-teal transition-colors mb-2">
                                {post.title}
                            </h4>
                            <div className="flex items-center gap-4 text-[11px] font-bold text-white/40 uppercase tracking-tighter">
