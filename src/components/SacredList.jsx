    ...
    return (
        <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-2 text-center pt-3 sm:pt-4">
                <h2 className="text-4xl sm:text-4xl font-black text-[#F3F4F6] tracking-tight">성지글 아카이브</h2>
            ...
            <div className="flex gap-2 p-1 bg-white/5 rounded-2xl border border-white/5">
                <button
                    ...
                    className={`flex-1 py-2.5 sm:py-3 text-sm sm:text-sm font-black rounded-xl transition ${filter === 'recent' ? 'bg-white text-black' : 'text-[#9CA3AF] hover:text-[#F3F4F6]'}`}
                >
                    최근 적중
                </button>
                <button
                    onClick={() => setFilter('popular')}
                    className={`flex-1 py-2.5 sm:py-3 text-sm sm:text-sm font-black rounded-xl transition ${filter === 'popular' ? 'bg-white text-black' : 'text-[#9CA3AF] hover:text-[#F3F4F6]'}`}
                >
                    정확도 높은 글
                </button>
            </div>
                ...
                {posts.map((post) => (
                    ...
                    >
                        <div className="relative">
                            ...
                            <div className="flex flex-wrap items-center gap-2.5 sm:gap-4 text-xs sm:text-xs font-bold text-[#9CA3AF] uppercase tracking-tighter">
