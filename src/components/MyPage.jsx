...
const MyPage = ({ onBack, onStockClick, onNavigatePath }) => {
                ...
                <div className="text-center pt-12 pb-8 space-y-6">
                    ...
                    <div className="flex justify-center">
                        <div className="relative">
                            <div className="w-20 h-20 bg-gradient-to-br from-neon-teal/10 to-neon-pink/10 
                    ...
                    <div className="space-y-4">
                        <h2 className="text-[26px] font-black text-[#F3F4F6] leading-tight">
                            내 예언을<br />관리하고 싶다면
                        </h2>
                        <p className="text-[15px] text-[#9CA3AF] leading-6 max-w-xs mx-auto break-keep">
                    ...
                    {/* Features */}
                    <div className="grid grid-cols-1 gap-2.5 max-w-sm mx-auto pt-3">
                        {[
                            ...
                            <div key={i} className="flex items-center gap-3 px-4 py-3 
                                ...
                                <span className="text-[14px] font-bold text-[#9CA3AF]">{f.text}</span>
                            </div>
                        ))}
                    </div>
                </div>
    ...
    return (
        <div className="w-full animate-in fade-in duration-700">
            {/* Profile Card */}
            <div className="crystal-glass rounded-[1.75rem] p-5 mb-5 border border-white/10 
                ...
                <div className="flex items-center gap-3.5 relative">
                    ...
                    <div className="relative shrink-0">
                        {profile?.avatar ? (
                            <img 
                                src={profile.avatar} 
                                alt="프로필" 
                                className="w-14 h-14 rounded-full border-2 border-neon-teal/30 object-cover"
                            />
                        ) : (
                            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-neon-teal/20 to-neon-pink/20 
                                            border-2 border-white/10 flex items-center justify-center">
                                <span className="text-xl font-black text-[#9CA3AF]">
                    ...
                    <div className="flex-1 min-w-0 pr-1">
                        <h3 className={`text-[16px] font-black text-neon-teal brightness-125 truncate`}>{profile?.nickname || '별지기'}</h3>
                        <p className="text-[13px] font-bold text-neon-teal/40 truncate">{profile?.email || '카카오 로그인'}</p>
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                            <span className="px-2 py-0.5 bg-neon-teal/10 border border-neon-teal/20 
                                             rounded-full text-[11px] font-black text-neon-teal uppercase tracking-wider shrink-0">
                                정식 별지기
                            </span>
                            <span className="text-[12px] font-bold text-neon-teal/30 shrink-0">
            ...
            {/* Notifications */}
            <div className="rounded-[1.35rem] border border-white/10 bg-white/5 p-4 mb-5 space-y-3">
                <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                            <p className="text-[14px] font-black text-neon-teal/70 uppercase tracking-[0.18em]">새 소식</p>
                            <p className="text-[12px] text-[#9CA3AF] truncate">기한 도래와 적중 흐름만 모아 보기</p>
                        </div>
                    </div>
                    <span className="text-[12px] font-bold text-neon-teal/40 shrink-0">
                        {notifications.filter(item => !item.is_read).length}개
                    </span>
                </div>

                {isLoadingNotifications ? (
                    <div className="py-4 text-center text-[14px] font-bold text-[#9CA3AF]">새 소식 불러오는 중</div>
                ) : notifications.length === 0 ? (
                    <div className="rounded-2xl border border-white/5 bg-[#121212]/20 px-4 py-4 text-center">
                        <p className="text-[14px] font-bold text-[#9CA3AF]">아직 새 소식이 없습니다</p>
                        <p className="text-[12px] text-[#6B7280] mt-1">기한 도래와 적중 알림이 여기에 도착합니다</p>
                    </div>
                ) : (
                        ...
                        {notifications.map((item) => (
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        <p className={`text-[14px] font-black ${item.is_read ? 'text-[#D1D5DB]' : 'text-[#F3F4F6]'}`}>{item.title}</p>
                                        <p className="text-[13px] text-[#9CA3AF] mt-1.5 leading-snug break-keep">{item.body}</p>
                                    </div>
                                    {!item.is_read ? (
            ...
            <div className="space-y-3">
                <div className="flex items-center justify-between px-1">
                    <h4 className="text-[14px] font-black text-neon-teal/60 uppercase tracking-[0.22em]">내 예언 기록</h4>
                    <span className="text-[13px] font-bold text-neon-teal/30">{predictions.length}건</span>
                </div>

                <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5 space-y-1.5">
                    <p className="text-[12px] font-black text-neon-teal/70 tracking-wider uppercase">판정 룰</p>
                    <p className="text-[13px] text-[#9CA3AF]">기한일 종가 기준 · <span className="text-joy-gold font-bold">1%</span> 적중 · <span className="text-joy-gold/90 font-bold">3%</span> 근접</p>
                    <button
                        type="button"
                        onClick={() => setIsGuideExpanded(prev => !prev)}
                        className="text-[13px] font-bold text-neon-teal/80 hover:text-neon-teal transition-colors pt-1"
                    >
                        {isGuideExpanded ? '접기 ▲' : '더보기 ▼'}
                    </button>
                    {isGuideExpanded && (
                        <div className="mt-1.5 rounded-xl border border-white/10 bg-[#121212]/20 px-3 py-3 space-y-2">
                            <p className="text-[12px] text-[#D1D5DB]">1. 기한일 종가로 최종 판정합니다.</p>
                            <p className="text-[12px] text-[#D1D5DB]">2. 오차율 = `|실제 종가 - 예언가| / 예언가 × 100`</p>
                            <p className="text-[12px] text-[#D1D5DB]">3. 5% 이내면 아슬아슬, 그 밖은 빗나감으로 봅니다.</p>
                            <p className="text-[12px] text-[#D1D5DB] break-keep">4. 근접 적중은 적중 그룹으로 집계되고, 기한 전에는 진행중으로 표시됩니다.</p>
                            <p className="text-[12px] text-[#9CA3AF] break-keep">시장 휴장/데이터 지연 시 실제 반영 시점이 늦어질 수 있습니다.</p>
                        </div>
                    )}
                </div>
                ...
                {isLoadingPreds ? (
                    ...
                    <div className="relative ml-3.5 pl-5 border-l border-white/10 space-y-6 pb-8">
                        {predictions.map((pred, idx) => {
                            ...
                            return (
                                <div key={pred.id} className="relative animate-in slide-in-from-left duration-500" style={{ delay: `${idx * 100}ms` }}>
                                    {/* Timeline Dot */}
                                    <div className={`absolute -left-[27px] top-5 w-3.5 h-3.5 rounded-full border-[3px] border-[#0a0a10] z-10 ${theme.dot}`}></div>
                                    
                                    {/* Date Header */}
                                    <div className="mb-2.5">
                                        <span className="text-[12px] font-black text-neon-teal/40 tracking-widest uppercase">{dateStr}</span>
                                    </div>

                                    {/* Card */}
                                    <div className={`group relative crystal-glass rounded-[1.5rem] p-4 border-[0.5px] transition-all duration-300 ${theme.bg} ${theme.border} hover:bg-white/10 ${theme.glow}`}>
                                        ...
                                        <div className="flex flex-row justify-between items-start gap-2 mb-4">
                                            ...
                                            >
                                                <div className="flex items-center gap-2 min-w-0 pr-1">
                                                    <span className={`text-[11px] font-black uppercase tracking-widest px-2.5 py-1.5 rounded-xl border ${theme.border} ${theme.color} bg-[#121212]/60 shadow-lg shadow-black/20 shrink-0`}>
                                                        {theme.label}
                                                    </span>
                                                    <span className={`text-[19px] font-black ${theme.color} brightness-125 truncate leading-tight`}>{pred.stockName}</span>
                                                </div>
                                                <div className="flex items-center gap-2 mt-1 min-w-0 pr-1">
                                                    <span className={`text-[13px] font-black ${theme.color} opacity-40 uppercase tracking-tighter bg-white/5 px-1.5 py-0.5 rounded border border-white/10 shrink-0`}>{pred.stockSymbol}</span>
                                                    <span className={`text-[13px] font-black ${theme.color} opacity-30 uppercase tracking-tighter shrink-0 ml-1`}>기한:</span>
                                                    <span className={`text-[13px] font-black ${theme.color} tracking-tight shrink-0`}>{targetDateStr}</span>
                                                </div>
                                            </div>
                                                ...
                                                {deletingId === pred.id ? (
                                                    <div className="flex flex-col gap-1.5 animate-in fade-in zoom-in duration-300">
                                                        <button 
                                                            ...
                                                            className="px-3 py-1.5 bg-neon-pink text-[#F3F4F6] text-[12px] font-black rounded-lg shadow-lg shadow-neon-pink/20"
                                                        >
                                                            삭제
                                                        </button>
                                                        <button 
                                                            ...
                                                            className="px-3 py-1.5 bg-white/10 text-[#9CA3AF] text-[12px] font-black rounded-lg"
                                                        >
                                                            취소
                                                        </button>
                                                    </div>
                                                    ...
                                                    <div className="flex flex-row items-center">
                                                        <button 
                                                            ...
                                                            className={`p-2 ${theme.color} opacity-40 hover:opacity-100 hover:bg-white/5 rounded-full transition-all shrink-0`}
                                        ...
                                        {deleteFeedbackId === pred.id && deleteFeedbackMessage ? (
                                            <p className="mb-3 text-[12px] font-bold text-[#9CA3AF]">
                                                {deleteFeedbackMessage}
                                            </p>
                                        ) : shareFeedbackId === pred.id && shareFeedbackMessage ? (
                                            <p className="mb-3 text-[12px] font-bold text-neon-teal animate-in fade-in duration-300">
                                                {shareFeedbackMessage}
                                            </p>
                                        ) : null}

                                        {/* Content: Price Comparison (Super High Clarity Grid) */}
                                        <div className="grid grid-cols-2 gap-px bg-white/5 border-[0.5px] border-white/10 rounded-xl overflow-hidden mb-4 shadow-xl">
                                            <div className="bg-[#121212]/40 p-3.5 space-y-1">
                                                <p className={`text-[11px] font-black ${theme.color} opacity-40 uppercase tracking-widest`}>현재가</p>
                                                {currentPriceValue > 0 ? (
                                                    <div className="flex items-baseline min-w-0">
                                                        <p className={`text-[20px] font-black font-brandEn ${theme.color} brightness-110 leading-none truncate`}>
                                                            {currentPriceValue.toLocaleString()}
                                                        </p>
                                                        <span className="text-[13px] ml-1 opacity-40 font-brandKo shrink-0">원</span>
                                                    </div>
                                                ) : (
                                                    <p className="text-[14px] font-black text-[#9CA3AF] leading-none shrink-0 py-1">시세 갱신 중</p>
                                                )}
                                                <p className="text-[10px] font-black text-[#9CA3AF] tracking-widest uppercase truncate mt-0.5">{currentPriceValue > 0 ? (pred.quoteStatusLabel || '최근값') : '로딩중'}</p>
                                            </div>
                                            <div className="bg-[#121212]/60 p-3.5 space-y-1 text-right flex flex-col items-end">
                                                <p className={`text-[11px] font-black ${theme.color} opacity-40 uppercase tracking-widest`}>목표가</p>
                                                <div className="flex items-baseline justify-end min-w-0 w-full mt-auto">
                                                    <p className={`text-[20px] font-black font-brandEn leading-none ${theme.color} brightness-125 truncate`}>
                                                        {Number(pred.price_target).toLocaleString()}
                                                    </p>
                                                    <span className="text-[13px] ml-1 opacity-40 font-brandKo shrink-0">원</span>
                                                </div>
                                            </div>
                                        ...
                                        <button 
                                            onClick={() => pred.stock && onStockClick?.(pred.stock)}
                                            className={`w-full flex items-center justify-between py-3 px-5 rounded-xl text-[14px] font-black ${theme.color} bg-white/5 hover:bg-white/10 border-[0.5px] ${theme.border} transition-all duration-300 group/btn shadow-md`}
                                        >
                                            <span className="tracking-tight">이 종목 다시 보기</span>
                                            <ChevronRight className={`w-[20px] h-[20px] group-hover:translate-x-1 transition-transform`} />
                                        </button>
                                    </div>
                                </div>
                            );
