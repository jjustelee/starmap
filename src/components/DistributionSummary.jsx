...
const DistributionSummary = ({
    ...
    const getPos = (val) => {
        const range = history.high52 - history.low52;
        if (range <= 0) return 50;
        const percent = ((val - history.low52) / range) * 100;
    ...
    if (isPioneer) {
        return (
            <div className="w-full relative mt-4 mb-6 pointer-events-auto transition-all duration-700">
                <div className="bg-[#141419]/90 backdrop-blur-[40px] border border-white/10 rounded-3xl p-6 relative overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_20px_40px_-10px_rgba(0,0,0,0.8)]">
                    ...
                    <div className="relative w-full pt-44 pb-4 mt-6">
                        <div className="relative w-full h-[4px] bg-white/10 rounded-full">
                            <div 
                                className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-1000 ease-out"
                                style={{ left: `${getPos(history.current)}%` }}
                            >
                                <div className="absolute bottom-full mb-4 flex flex-col items-center w-max pb-12">
                                    <span className="text-[11px] font-black text-neon-teal mb-2 bg-neon-teal/10 px-3 py-1 rounded-full uppercase">현재가 위치</span>
                                    <span className="text-[36px] font-extrabold font-brandEn text-white tracking-tighter antialiased">{history.current.toLocaleString()}</span>
                                </div>
                        ...
                        <div className="flex justify-between items-start mt-4 px-1">
                            <div className="flex flex-col"><span className="text-[10px] font-black text-history-low uppercase mb-1">52주 최저</span><span className="text-[15px] font-bold font-brandEn text-history-low/80">{history.low52.toLocaleString()}</span></div>
                            <div className="flex flex-col items-end"><span className="text-[10px] font-black text-history-high uppercase mb-1">52주 최고</span><span className="text-[15px] font-bold font-brandEn text-history-high/80">{history.high52.toLocaleString()}</span></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="w-full relative mt-4 mb-6 pointer-events-auto transition-all duration-700">
            <div className="bg-[#141419]/90 backdrop-blur-[40px] border border-white/10 rounded-3xl p-6 sm:p-8 relative overflow-hidden shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_20px_40px_-10px_rgba(0,0,0,0.8)]">
                ...
                <div className="relative w-full pt-32 pb-6 mt-4 px-2">
                    <div className="relative w-full h-[3px] bg-white/10 rounded-full">
                        {/* 52 Week Low Marker */}
                        <div className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center opacity-70 sm:opacity-100" style={{ left: `${getPos(history.low52)}%` }}>
                            ...
                            <div className="absolute top-6 flex flex-col items-center w-max opacity-80">
                                ...
                                <span className="text-[12px] font-bold font-brandEn text-history-low">{history.low52.toLocaleString()}</span>
                            </div>
                        </div>
                        {/* 52 Week High Marker */}
                        <div className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center opacity-70 sm:opacity-100" style={{ left: `${getPos(history.high52)}%` }}>
                            ...
                            <div className="absolute top-6 flex flex-col items-center w-max opacity-80">
                                ...
                                <span className="text-[12px] font-bold font-brandEn text-history-high">{history.high52.toLocaleString()}</span>
                            </div>
                        </div>

                        {/* Average Marker */}
                        <div className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-1000 ease-out z-10" style={{ left: `${getPos(consensus.avg)}%` }}>
                            <div className="absolute bottom-full mb-4 flex flex-col items-center w-max pb-10">
                                ...
                                <span className="text-[20px] font-bold font-brandEn text-white antialiased">{consensus.avg.toLocaleString()}</span>
                            </div>
                        ...
                        {/* Mode Marker */}
                        <div className="absolute top-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center transition-all duration-1000 ease-out z-20" style={{ left: `${getPos(consensus.mode)}%` }}>
                            <div className="absolute bottom-full mb-4 flex flex-col items-center w-max pb-24">
                                ...
                                <span className="text-[40px] sm:text-[44px] font-extrabold font-brandEn text-white tracking-tighter leading-none antialiased">{consensus.mode.toLocaleString()}</span>
                            </div>
                    ...
                    <div className="flex justify-between items-start mt-6 italic">
                        <div className="flex flex-col items-start translate-y-2" style={{ paddingLeft: `${getPos(consensus.min)}%` }}>
                            <span className="text-[10px] font-black text-pred-min uppercase tracking-widest">최저 예측</span>
                            <span className="text-[14px] font-bold font-brandEn text-pred-min opacity-80">{consensus.min.toLocaleString()}</span>
                        </div>
                        <div className="flex flex-col items-end translate-y-2" style={{ paddingRight: `${100 - getPos(consensus.max)}%` }}>
                            <span className="text-[10px] font-black text-pred-max uppercase tracking-widest">최고 예측</span>
                            <span className="text-[14px] font-bold font-brandEn text-pred-max opacity-80">{consensus.max.toLocaleString()}</span>
                        </div>
                    </div>
                </div>
            </div>
